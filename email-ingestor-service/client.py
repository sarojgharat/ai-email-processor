...
from db_client import create_email_request, update_email_request
...
async def invoke_agent(base_url, query, conversation_id) -> SendMessageResponse:
    async with httpx.AsyncClient(timeout=httpx.Timeout(120.0)) as httpx_client:
        ...
        try:
            ...
            raise RuntimeError(
                f'Failed to fetch the public agent card. Cannot continue: {e}', exc_info=True
            ) from e
...
async def main():
    ...
    #await read_and_process_emails(business_process, max_emails=3)

    business_process = "equipment"
    await read_and_process_emails(business_process, max_emails=6)
